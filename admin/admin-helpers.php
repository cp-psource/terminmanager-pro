...
function _appointments_get_admin_notice( $slug ) {
	...
	$notices = array(
		'1-7-gcal' => sprintf(
			_x( '%s haben sich in Version 1.7 geändert. WennDu Google Kalender vor 1.7 verwendet hast, überprüfe bitte Deine Einstellungen.', 'Hinweis für Administratoren der Google Kalendereinstellungen für das Upgrade auf 1.7.', 'appointments' ),
			'<a href="' . esc_url( $gcal_tab_url ) . '">' . __( 'Google Kalendar Einstellungen', 'appointments' ) . '</a>'
		),
	);
...
function _appointments_settings_submit_block( $tab, $text = '', $class = 'primary' ) {
	if ( ! $text ) {
		$text = __( 'Änderungen speichern', 'appointments' );
	}
...
function _appointments_html_chceckbox( $options, $name, $slave = '' ) {
	...
	printf(
		...
		esc_attr( $name ),
		esc_attr( 'yes' ),
		esc_attr__( 'Ja', 'appointments' ),
		esc_attr__( 'Nein', 'appointments' ),
		esc_attr( $slave ),
		$checked
	);
}
