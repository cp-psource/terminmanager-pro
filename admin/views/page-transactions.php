<div class='wrap'>
	<h2><?php echo __('Transaktionen','appointments'); ?></h2>

	<ul class="subsubsub">
		<li><a href="<?php echo add_query_arg('type', 'past'); ?>" class="rbutton <?php if($type == 'past') echo 'current'; ?>"><?php  _e('Kürzliche Transaktionen', 'appointments'); ?></a> | </li>
		<li><a href="<?php echo add_query_arg('type', 'pending'); ?>" class="rbutton <?php if($type == 'pending') echo 'current'; ?>"><?php  _e('Ausstehende Transaktionen', 'appointments'); ?></a> | </li>
		<li><a href="<?php echo add_query_arg('type', 'future'); ?>" class="rbutton <?php if($type == 'future') echo 'current'; ?>"><?php  _e('Zukünftige Transaktionen', 'appointments'); ?></a></li>
	</ul>
				...
				?>
					...
					<tr valign="middle" class="alternate">
						...
						<td class="column-transid">
							<?php echo ! empty( $transaction->transaction_paypal_ID ) ? $transaction->transaction_paypal_ID : __('Noch keine','appointments'); ?>
						</td>
						<td class="column-status">
							<?php echo ! empty( $transaction->transaction_status ) ? $transaction->transaction_status : __('Noch keine','appointments'); ?>
						</td>
					</tr>
				<?php endforeach; ?>
			<?php else: ?>
				<tr valign="middle" class="alternate" >
					<td colspan="<?php echo count($columns); ?>" scope="row"><?php _e('Es wurden keine Transaktionen gefunden, Geduld ist eine Tugend.','appointments'); ?></td>
				</tr>
			<?php endif; ?>
		</tbody>
	</table>
