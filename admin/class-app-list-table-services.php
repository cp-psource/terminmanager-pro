...
class Appointments_WP_List_Table_Services extends WP_List_Table {
	...
	public function column_capacity( $item ) {
		if ( 0 === $item->capacity ) {
			return __( 'Begrenzt durch die Anzahl der Dienstleister', 'appointments' );
		}
	...
	public function get_columns() {
		$columns = array(
			...
			'capacity' => __( 'Kapazität', 'appointments' ),
			'duration' => __( 'Dauer', 'appointments' ),
			'price' => sprintf( __( 'Preis (%s)', 'appointments' ), $this->currency ),
	...
	public function get_bulk_actions() {
		$actions = array(
			'delete' => __( 'Löschen', 'appointments' ),
		);
		return $actions;
	}
