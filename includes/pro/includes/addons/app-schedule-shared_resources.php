<?php
/*
Plugin Name: Gemeinsame Ressourcen
Description: Ermöglicht Deinen Diensten, gemeinsam genutzte reale Ressourcen wie Räume oder Fahrzeuge zu definieren. Die Dienste, die Ressourcen gemeinsam nutzen, ermöglichen nur Termine bis zur minimalen gemeinsamen Kapazität.
...
class App_Schedule_SharedResources {
	...
	private function _add_hooks() {
		...
		add_action( 'psource_appointments_update_service', array( $this, 'save_service_shared_resources' ) );
		/**
	...
	public function add_service_selection( $out, $service_id ) {
		...
		$out .= '<h4>' . __( 'Teilt Ressourcen mit', 'appointments' ) . '</h4>';
		foreach ( $all as $service ) {
			...
			$data_on = esc_attr( sprintf( 'Teilt mit %s', 'appointments' ), $service->name );
	...
	public function add_new_service_selection() {
		...
		$all        = appointments_get_services();
		?>
			<th scope="row"><?php _e( 'Teilt Ressourcen mit', 'appointments' ); ?></th>
...
foreach ( $all as $service ) {
	...
	$data_on = sprintf( __( 'Teilt mit "%s"', 'appointments' ), $service->name );
	$disabled = in_array( $service->ID, $shared_ids ) && ! in_array( $service->ID, $direct_ids ) ? 'disabled="disabled"' : '';
?>
...
?>
	...
	public function add_columns( $columns ) {
		$columns['shared'] = __( 'Geteilte Ressourcen', 'appointments' );
		return $columns;
	}
	...
	public function get_column_shared( $content, $service ) {
		...
		if ( empty( $content ) ) {
			$content = __( 'Keine gemeinsam genutzten Ressourcen', 'appointments' );
		} else {
