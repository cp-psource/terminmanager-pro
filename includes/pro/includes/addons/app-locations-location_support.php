<?php
/*
Plugin Name: Standorte
Description: Ermöglicht das Erstellen von Standorten für Deine Termine.
...
class App_Locations_LocationsWorker {
	...
	public function show_appointment_location ($deprecated, $appointment) {
		...
		$editable .= '<span class="title">' . __('Standort', 'appointments') . '</span>';
	...
	public function include_scripts () {
		...
		wp_localize_script("app-locations", '_app_locations_data', apply_filters('app-locations-location_model_template', array(
			'model' => array(
				...
				'labels' => array(
					'add_location' => __('Hinzufügen', 'appointments'),
					'save_location' => __('Speichern', 'appointments'),
					'new_location' => __('Erstelle einen neuen Standort', 'appointments'),
					'edit_location' => __('Standort bearbeiten', 'appointments'),
					'cancel_editing' => _x('Abbrechen', 'Aktuelle Aktion löschen', 'appointments'),
				),
			),
		)));
	}
	...
	public function add_settings_sections( $sections ) {
		$sections['locations'] = array(
			'locations' => __( 'Standorte bearbeiten', 'appointments' ),
			'settings' => __( 'Einstellungen', 'appointments' )
		);
	...
	public function add_settings_tab( $tabs ) {
		$tabs['locations'] = __( 'Standorte', 'appointments' );
		return $tabs;
	}
	...
	public function settings_tab_add ($tabs) {
		...
		foreach ($tabs as $key => $label) {
			if ($key == self::INJECT_TAB_BEFORE) {
				$ret[self::SETTINGS_TAB] = __('Standorte', 'appointments');
			}
			$ret[$key] = $label;
		}
		return $ret;
					...
					<div class="form-wrap">
						<form action="" method="post" id="add-location">
							<?php if ( isset( $_GET['error'] ) ): ?>
								<div class="error">
									<p><?php _e( 'Die Adresse darf nicht leer sein', 'appointments' ); ?></p>
								</div>
							<?php endif; ?>

							<h2><?php _e( 'Neuen Standort hinzufügen', 'appointments' ); ?></h2>

							<div class="form-field form-required">
								<label for="location-add"><?php _e( 'Addresse', 'appointments' ); ?></label>
								<input type="text" id="location-add" name="location" value="">
							</div>

							<?php _appointments_settings_submit_block( 'add_locations', __( 'Standort hinzufügen', 'appointments' ) ); ?>
						</form>

						<form action="" method="post" id="edit-location" class="hidden">
							<h2><?php _e( 'Standort bearbeiten', 'appointments' ); ?></h2>

							<div class="form-field form-required">
								<label for="location-edit"><?php _e( 'Addresse', 'appointments' ); ?></label>
								<input type="text" id="location-edit" name="location" value="">
							</div>
							<input type="hidden" id="location-id" name="location_id" value="">

							<?php _appointments_settings_submit_block( 'edit_locations', __( 'Standort bearbeiten', 'appointments' ) ); ?>
						</form>
						...
						<script>
							jQuery(document).ready( function( $ ) {
								...
								$('.delete-location').on('click', function( e ) {
									return confirm( '<?php _e( "Bist Du sicher, dass Du diesen Speicherort löschen möchtest?", "appointments" ); ?>');
								});
							});
						</script>
	...
	public function settings_settings_section() {
		...
		<form method="post" action="" >
			<table class="form-table">
				<tr>
					<th scope="row"><label for="locations_settings-my-appointments"><?php _e('Zeige meinen Terminort an', 'appointments')?></label></th>
					<td>
						<select id="locations_settings-my-appointments" name="locations_settings[my_appointments]" autocomplete="off">
							<option value=""></option>
							<option value="after_service" <?php selected($this->_data['locations_settings']['my_appointments'], 'after_service'); ?> ><?php _e('Automatisch nach dem Service', 'appointments'); ?></option>
							<option value="after_worker" <?php selected($this->_data['locations_settings']['my_appointments'], 'after_worker'); ?> ><?php _e('Automatisch nach Provider', 'appointments'); ?></option>
							<option value="after_date" <?php selected($this->_data['locations_settings']['my_appointments'], 'after_date'); ?> ><?php _e('Automatisch nach Datum/Uhrzeit', 'appointments'); ?></option>
							<option value="after_status" <?php selected($this->_data['locations_settings']['my_appointments'], 'after_status'); ?> ><?php _e('Automatisch nach Status', 'appointments'); ?></option>
						</select>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label for="locations_settings-all-appointments"><?php _e('Alle Termin-Standorte anzeigen', 'appointments')?></label></th>
					<td>
						<select id="locations_settings-all-appointments" name="locations_settings[all_appointments]" autocomplete="off">
							<option value=""></option>
							<option value="after_service" <?php selected($this->_data['locations_settings']['all_appointments'], 'after_service'); ?> ><?php _e('Automatisch nach dem Service', 'appointments'); ?></option>
							<option value="after_provider" <?php selected($this->_data['locations_settings']['all_appointments'], 'after_provider'); ?> ><?php _e('Automatisch nach Provider', 'appointments'); ?></option>
							<option value="after_client" <?php selected($this->_data['locations_settings']['all_appointments'], 'after_client'); ?> ><?php _e('Automatisch nach Client', 'appointments'); ?></option>
							<option value="after_date" <?php selected($this->_data['locations_settings']['all_appointments'], 'after_date'); ?> ><?php _e('Automatisch nach Datum/Uhrzeit', 'appointments'); ?></option>
							<option value="after_status" <?php selected($this->_data['locations_settings']['all_appointments'], 'after_status'); ?> ><?php _e('Automatisch nach Status', 'appointments'); ?></option>
						</select>
					</td>
				</tr>
			</table>
	...
	public function my_appointments_headers ($headers) {
		...
		$rx = '(' .
			...
			preg_quote('</th>', '/') .
		')';
		$location = '<th class="my-appointments-location">' . __('Standort', 'appointments') . '</th>';
	...
	public function all_appointments_headers ($headers) {
		...
		$rx = '(' .
			...
			preg_quote('</th>', '/') .
		')';
		$location = '<th class="all-appointments-location">' . __('Standort', 'appointments') . '</th>';
		return preg_replace("/{$rx}/", '\1' . $location, $headers);
	}
