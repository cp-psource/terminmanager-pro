...
class Appointments_Sessions {
	...
	public static function get_current_visitor_key() {
		...
		if ( false === $key ) {
			$key = $_COOKIE['psource_appointments'];
		...
		if ( $set_cookie ) {
			@setcookie( 'psource_appointments', $key, self::get_cookie_expiration_time(), self::get_cookie_path(), self::get_cookie_domain() );
		}
		return $key;
	}
	...
	public static function is_visitor_appointments_cookie_set() {
		if ( isset( $_COOKIE['psource_appointments_userdata'] ) ) {
			@setcookie( 'psource_appointments_userdata', null, -1, self::get_cookie_path(), self::get_cookie_domain() );
		}
		return isset( $_COOKIE['psource_appointments'] );
	}
	...
	public static function clear_visitor_data() {
		@setcookie( 'psource_appointments', null, -1, self::get_cookie_path(), self::get_cookie_domain() );
	}
