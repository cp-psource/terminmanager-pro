...
if ( ! class_exists( 'App_Shortcode' ) ) {
	abstract class App_Shortcode extends App_Codec_Instance {
		...
		public function get_shortcode_help ($all='') {
			...
			$args_help = '';
			if (!empty($arguments)) {
				$args_help .= '<h4>' . __('Argumente', 'appointments') . '</h4><dl>';
			...
			$help = '' .
			        ...
			        '<h4>' . __('Beschreibung', 'appointments'). '</h4>' .
			        $usage .
			        $args_help .
			        '</div>' .
			        '</div>' .
		...
		protected function _to_help_string ($arg, $help) {
			...
			if (!empty($help['allowed_values'])) {
				$ret[] = __('Zulässige Werte:', 'appointments') . ' <code>' . join('</code>, <code>', $help['allowed_values']) . '</code>';
			}
			if (!empty($help['example'])) {
				$ret[] = __('Beispiel:', 'appointments') . ' <code>[' . $this->_key . ' ... ' . $arg . '="' . $help['example'] . '"]</code>';
			}
			return join('<br />', $ret);
		}
